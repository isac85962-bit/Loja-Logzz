...
st.markdown("""
    <style>
    .stButton>button { width: 100%; border-radius: 10px; background-color: #FF8C00; color: white; font-weight: bold; border: none; height: 40px; }
    .stButton>button:hover { background-color: #FFA500; color: black; }
    h1 { color: #FF8C00; text-align: center; }
    .produto-container { text-align: center; padding: 15px; border: 1px solid #333; border-radius: 15px; margin-bottom: 25px; background-color: #111; }
    </style>
    """, unsafe_allow_html=True)
...
# 7. Suporte via WhatsApp
st.sidebar.title("Top Ofertas")
meu_whatsapp = "5547997270179" 
link_whatsapp = f"https://wa.me/{meu_whatsapp}?text=Olá! Gostaria de tirar uma dúvida sobre um produto."
st.sidebar.markdown(f'<a href="{link_whatsapp}" target="_blank"><button style="width:100%; border-radius:10px; background-color:#25D366; color:white; font-weight:bold; border:none; height:45px; cursor:pointer;">Falar no WhatsApp 💬</button></a>', unsafe_allow_html=True)